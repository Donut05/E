	...
	if self.currentState then
		...
		if (self.saved.raider or g_unitManager:sv_getHostSettings().aggroCreations) and self.currentState ~= self.rangedAttack and self.target == nil and self.suppressionFireTimer:done() then
			...
			if self.randomRaidFireTimer:done() then
				local attackableShape, attackPosition = FindAttackableShape(self.unit.character.worldPosition,
					self.attackRange, self.projectileBreachLevel)
...
function TapebotUnit.updateAim(self, dt)
	if self.target and sm.exists(self.target) and type(self.target) == "Character" and not self.target:isDowned() then
		local success, aimPoint = sm.ai.getAimPosition(self.unit.character, self.target, self.attackRange,
			self.fireLaneWidth)
		if success then
			...
			if self.predictedVelocity:length() > 0 and self.target:getVelocity():length() > self.predictedVelocity:length() then
				self.predictedVelocity = magicPositionInterpolation(self.predictedVelocity, self.target:getVelocity(), dt,
					1.0 / 10.0)
			else
...
function TapebotUnit.server_onUnitUpdate(self, dt)
		...
		for _, allyUnit in ipairs(sm.unit.getAllUnits()) do
			if sm.exists(allyUnit) and self.unit ~= allyUnit and allyUnit.character and isAnyOf(allyUnit.character:getCharacterType(), g_robots) and InSameWorld(self.unit, allyUnit) then
				...
				if inAllyRange or InSameGroup(allyUnit, self.unit) then
					...
					if sameTeam then
						sm.event.sendToUnit(allyUnit, "sv_e_receiveTarget",
							{ targetCharacter = targetCharacter, sendingUnit = self.unit })
					end
				end
			end
		end
	...
	if self.saved.raidPosition then
		local flatFromToRaid = sm.vec3.new(self.saved.raidPosition.x, self.saved.raidPosition.y,
			self.unit.character.worldPosition.z) - self.unit.character.worldPosition
	...
	if self.ambushPosition then
		...
		local flatFromToAmbush = sm.vec3.new(self.ambushPosition.x, self.ambushPosition.y,
			self.unit.character.worldPosition.z) - self.unit.character.worldPosition
			...
			elseif self.roamTimer:done() and not (self.currentState == self.idleState and result == "started") then
				...
				if self.saved.patrolling then
					...
					elseif self.currentState ~= self.roamState and self.currentState ~= self.positioningState then
						self.positioning.desiredPosition = self.unit.character.worldPosition
						self.positioning.desiredDirection = self.unit.character.direction:rotateZ(math.random() * 2 *
						math.pi)
						self.currentState = self.positioning
					end
				else
					self.positioning.desiredPosition = self.unit.character.worldPosition
					self.positioning.desiredDirection = self.unit.character.direction:rotateZ(math.random() * 2 * math
					.pi)
					self.currentState = self.positioning
				end
...
function TapebotUnit.server_onProjectile(self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal,
										 projectileUuid)
	...
	if type(attacker) == "Player" or (type(attacker) == "Shape" and not isTrapProjectile(projectileUuid)) or teamOpponent then
		if damage > 0 then
			...
			if self:nearHead(hitPos) then
				...
				sm.effect.playEffect("TapeBot - Hit", hitPos, sm.vec3.zero(),
					sm.vec3.getRotation(ZAxis, hitVelocity:normalize()))
...
function TapebotUnit.server_onCollision(self, other, collisionPosition, selfPointVelocity, otherPointVelocity,
										collisionNormal)
	if not sm.exists(self.unit) then
		return
	end
	...
	local damage, tumbleTicks, tumbleVelocity, impactReaction = CharacterCollision(self.unit.character, other,
		collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal,
		self.saved.stats.maxhp / collisionDamageMultiplier)
...
function TapebotUnit.sv_spawnParts(self, impact)
	...
	local rightArmShape = rightArmBody:createPart(obj_robotparts_tapebotshooter, sm.vec3.new(1, 2, 0),
