...
function TotebotGreenUnit.server_onUnitUpdate(self, dt)
	...
	if self.saved.raidPosition and not self.ambushPosition and not self.lastTargetPosition and not self.target then
		local attackableShape, attackPosition = FindAttackableShape(self.saved.raidPosition, RAIDER_AMBUSH_RADIUS,
			MeleeBreachLevel)
	...
	if self.isInCombat and self.currentState ~= self.breachState then
		...
		local leveledNextTargetPosition = sm.vec3.new(nextTargetPosition.x, nextTargetPosition.y,
			self.unit.character.worldPosition.z)
		local valid, breachPosition, breachObject = sm.ai.getBreachablePosition(self.unit, leveledNextTargetPosition,
			breachDepth, MeleeBreachLevel)
...
function TotebotGreenUnit.server_onProjectile(self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData,
											  hitNormal, projectileUuid)
...
function TotebotGreenUnit.server_onCollision(self, other, collisionPosition, selfPointVelocity, otherPointVelocity,
											 collisionNormal)
	if not sm.exists(self.unit) then
		return
	end
...
function TotebotGreenUnit.sv_spawnParts(self, impact)
	...
	if SurvivalGame then
		if math.random(1, 5) == 1 then
			...
			local headShape = headBody:createPart(obj_interactive_robotbliphead01, sm.vec3.new(0, 1, 2),
