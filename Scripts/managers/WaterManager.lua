...
function WaterManager.onCellLoaded(self, x, y)
	...
	if #waterNodes > 0 then
		for _, node in ipairs(waterNodes) do
			local areaTrigger = sm.areaTrigger.createBoxWater(node.scale * 0.5, node.position, node.rotation,
				sm.areaTrigger.filter.character + sm.areaTrigger.filter.dynamicBody + sm.areaTrigger.filter.areaTrigger,
				{ water = true })
...
local function CalulateForce(waterHeightPos, worldPosition, worldRotation, velocity, halfExtent, mass)
	...
	local bottomArea = 0.5 *
	((corners[1] - corners[2]):cross(corners[1] - corners[3]):length() + (corners[4] - corners[2]):cross(corners[4] - corners[3]):length())
...
local function UpdateCharacterInWater(trigger, character)
	...
	if not character:isSwimming() then
		local depthScale = 1 -
		math.max(
		math.min(((character.worldPosition.z + characterDiveOffset) - waterHeightPos) / (characterDiveOffset * 2), 1.0),
			0.0)
	...
	if character:isTumbling() then
		...
		elseif (characterFloatHeight < waterHeightPos and characterFloatHeight > waterHeightFloatThreshold) then
			...
			local buoyancyForce = fluidDensity * displacedVolume * GRAVITY
			local diveDepthScale = 1 -
			math.max(
			math.min(
			(characterFloatHeight - waterHeightFloatThreshold) / (waterHeightPos - waterHeightFloatThreshold), 1.0), 0.0)
...
local function isOneOf(uuid, checklist)
...
local checklist = {
	...
	sm.uuid.new("d4e6c84c-a493-44b1-81aa-4f4741ea3ed8"),
	sm.uuid.new("b3402155-e8a3-452d-b6fb-55c8d4e4e681")
}
		...
		if sm.exists(result) then
			...
			elseif type(result) == "Body" then
				...
				for i, shape in pairs(shapes) do
					if isOneOf(shape.uuid, checklist) then
						sm.event.sendToInteractable(shape.interactable, "setIsInWater", true)
					end
				end
			end
		end
	...
	for _, result in ipairs(results) do
		if sm.exists(result) then
			...
			elseif type(result) == "Body" then
				...
				for i, shape in pairs(shapes) do
					if isOneOf(shape.uuid, checklist) then
						sm.event.sendToInteractable(shape.interactable, "setIsInWater", false)
					end
				end
			end
		end
...
function WaterManager.trigger_onStayWater(self, trigger, results)
	...
	for _, result in ipairs(results) do
		if sm.exists(result) then
			...
			elseif type(result) == "Body" then
				if result:hasChanged(sm.game.getCurrentTick() - 2) then
					print("BODY CHANGED! CHECKING FOR CHESTS!")
					local shapes = result:getShapes()
					for i, shape in pairs(shapes) do
						if isOneOf(shape.uuid, checklist) then
							sm.event.sendToInteractable(shape.interactable, "setIsInWater", true)
...
function WaterManager.trigger_onProjectile(self, trigger, hitPos, hitTime, hitVelocity, _, attacker, damage, userData,
