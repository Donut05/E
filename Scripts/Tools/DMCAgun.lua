...
function DMCAgun.loadAnimations( self )
...
function DMCAgun.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets DMCAgun renderable, change this to change the mesh
...
function DMCAgun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function DMCAgun.onAim( self, aiming )
	self.aiming = aiming
...
function DMCAgun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function DMCAgun.onShoot( self, dir )
...
function DMCAgun.calculateFirePosition( self )
...
function DMCAgun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function DMCAgun.calculateFpMuzzlePos( self )
...
function DMCAgun.cl_onSecondaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
			...
			if owner then
				sm.projectile.projectileAttack( sm.uuid.new("6bb9aa22-8b36-4fed-a740-028e5360a617"), 50, firePos, dir * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf )
			end
...
function DMCAgun:client_onFixedUpdate()
	...
	if self.aiming then
		...
		local position, rotation = self.tool:getTpBonePos( "pejnt_barrel" ), sm.camera.getRotation()
		self.network:sendToServer( "sv_syncFlames", position, rotation )
		if self.tool:isInFirstPersonView() then
